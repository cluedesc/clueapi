...
TEST_F(response_tests, text_response_constructor) {
    text_response_t res("Hello", status_t::e_status::ok, {{"X-Custom", "Value"}});

    EXPECT_EQ(res.body(), "Hello");
    EXPECT_EQ(res.status(), status_t::e_status::ok);
...
TEST_F(response_tests, json_response_sets_content_type_and_body) {
    json_response_t res(R"({"a":1})", status_t::e_status::created);

    EXPECT_EQ(res.headers().at("Content-Type"), "application/json");
    EXPECT_EQ(res.body(), "\"{\\\"a\\\":1}\"");
    EXPECT_EQ(res.status(), status_t::e_status::created);
}

TEST_F(response_tests, json_response_with_empty_body) {
    json_response_t res("", status_t::e_status::ok);
...
TEST_F(response_tests, redirect_response_with_valid_status) {
    redirect_response_t res("/new-location", status_t::e_status::moved_permanently);

    EXPECT_TRUE(res.body().empty());

    EXPECT_EQ(res.status(), status_t::e_status::moved_permanently);
    EXPECT_EQ(res.headers().at("Location"), "/new-location");
}

TEST_F(response_tests, redirect_response_defaults_invalid_status_to_found) {
    redirect_response_t res("/another-place", status_t::e_status::ok);

    EXPECT_EQ(res.status(), status_t::e_status::found);
...
TEST_F(file_response_tests, for_existing_file) {
    ...
    const auto expected_etag = fmt::format("\"{}\"", fmt::format("{}-{}", last_write, expected_size));
...
TEST_F(file_response_tests, for_non_existent_file) {
    ...
    EXPECT_EQ(res.status(), status_t::e_status::not_found);

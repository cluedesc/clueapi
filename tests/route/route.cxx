...
TEST_F(route_handler_test, sync_route) {
    ...
    route_t sync_route(types::method_t::e_method::get, "/sync", std::move(sync_handler));
...
TEST_F(route_handler_test, async_route) {
    ...
    route_t async_route(types::method_t::e_method::get, "/async", std::move(async_handler));
...
TEST_F(route_handler_test, move_semantics) {
    ...
    route_t route1(types::method_t::e_method::get, "/path", std::move(sync_handler));
...
TEST_F(radix_tree_tests, insert_and_find_root) {
    tree.insert(types::method_t::e_method::get, "/", [](ctx_t) { return types::text_response_t("root"); });

    auto result = tree.find(types::method_t::e_method::get, "/");
...
TEST_F(radix_tree_tests, insert_and_find_static_route) {
    tree.insert(types::method_t::e_method::get, "/hello/world", [](ctx_t) {
        return types::text_response_t("hello world");
    });

    auto result = tree.find(types::method_t::e_method::get, "/hello/world");

    ASSERT_TRUE(result.has_value());

    EXPECT_EQ(result->first(make_ctx({})).body(), "hello world");

    auto not_found = tree.find(types::method_t::e_method::get, "/hello");

    EXPECT_FALSE(not_found.has_value());
}

TEST_F(radix_tree_tests, node_splitting) {
    tree.insert(types::method_t::e_method::get, "/team", [](ctx_t) { return types::text_response_t("team"); });
    tree.insert(types::method_t::e_method::get, "/teams", [](ctx_t) { return types::text_response_t("teams"); });

    auto res1 = tree.find(types::method_t::e_method::get, "/team");

    ASSERT_TRUE(res1.has_value());

    EXPECT_EQ(res1->first(make_ctx({})).body(), "team");

    auto res2 = tree.find(types::method_t::e_method::get, "/teams");
...
TEST_F(radix_tree_tests, InsertAndFindDynamicRoute) {
    tree.insert(types::method_t::e_method::get, "/users/{id}", [](ctx_t ctx) {
        return types::text_response_t("user " + ctx.params().at("id"));
    });

    auto result = tree.find(types::method_t::e_method::get, "/users/123");
...
TEST_F(radix_tree_tests, prioritize_static_route) {
    tree.insert(types::method_t::e_method::get, "/users/profile", [](ctx_t) {
    ...
    auto res_dynamic = tree.find(types::method_t::e_method::get, "/users/abc");

    ASSERT_TRUE(res_dynamic.has_value());

    EXPECT_EQ(res_dynamic->first(make_ctx(res_dynamic->second)).body(), "user abc");

    auto res_static = tree.find(types::method_t::e_method::get, "/users/profile");
...
TEST_F(radix_tree_tests, different_methods_with_same_path) {
    tree.insert(types::method_t::e_method::get, "/resource", [](ctx_t) { return types::text_response_t("GET"); });
    tree.insert(types::method_t::e_method::post, "/resource", [](ctx_t) { return types::text_response_t("POST"); });

    auto res_get = tree.find(types::method_t::e_method::get, "/resource");

    ASSERT_TRUE(res_get.has_value());

    EXPECT_EQ(res_get->first(make_ctx()).body(), "GET");

    auto res_post = tree.find(types::method_t::e_method::post, "/resource");

    ASSERT_TRUE(res_post.has_value());

    EXPECT_EQ(res_post->first(make_ctx()).body(), "POST");

    auto res_put = tree.find(types::method_t::e_method::put, "/resource");

    EXPECT_FALSE(res_put.has_value());
}

TEST_F(radix_tree_tests, duplicate_route_throws_exception) {
    tree.insert(types::method_t::e_method::get, "/duplicate", [](ctx_t) { return types::text_response_t("first"); });

    EXPECT_THROW(
        tree.insert(
            types::method_t::e_method::get, "/duplicate", [](ctx_t) { return types::text_response_t("second"); }
        ),
...
TEST_F(radix_tree_tests, multiple_dynamic_parameters) {
    tree.insert(types::method_t::e_method::get, "/users/{userId}/posts/{postId}", [](ctx_t ctx) {
        return types::text_response_t("user " + ctx.params().at("userId") + " post " + ctx.params().at("postId"));
    });

    auto result = tree.find(types::method_t::e_method::get, "/users/123/posts/abc");
...
TEST_F(radix_tree_tests, complex_node_splitting) {
    tree.insert(types::method_t::e_method::get, "/content", [](ctx_t) { return types::text_response_t("content"); });
    tree.insert(types::method_t::e_method::get, "/contact", [](ctx_t) { return types::text_response_t("contact"); });

    auto res1 = tree.find(types::method_t::e_method::get, "/content");

    ASSERT_TRUE(res1.has_value());

    EXPECT_EQ(res1->first(make_ctx()).body(), "content");

    auto res2 = tree.find(types::method_t::e_method::get, "/contact");
...
TEST_F(radix_tree_tests, ambiguous_dynamic_routes_throw_exception) {
    tree.insert(types::method_t::e_method::get, "/users/{id}", [](ctx_t) { return types::text_response_t("id"); });

    EXPECT_THROW(
        tree.insert(
            types::method_t::e_method::get, "/users/{uuid}", [](ctx_t) { return types::text_response_t("uuid"); }
        ),

...
class add_header_middleware : public c_base_middleware {
   public:
    ...
    shared::awaitable_t<types::response_t> handle(
        const types::request_t& request, next_t next) override {
   ...
   public:
    shared::awaitable_t<types::response_t> handle(
        ...
        if (request.header("X-Short-Circuit").has_value()) {
            types::text_response_t res("short-circuited", types::status_t::ok);

            res.headers()["X-Body"] = "short-circuited";
...
class middleware_tests : public ::testing::Test {
   protected:
    ...
    shared::awaitable_t<types::response_t> final_handler(const types::request_t& request) {
        types::text_response_t res("final handler response");

        res.headers()["X-Body"] = "final handler response";

        co_return res;
    }

    next_t compose(
        const std::vector<std::shared_ptr<c_base_middleware>>& middlewares, next_t final_h) {
        auto chain = final_h;

        for (auto it = middlewares.rbegin(); it != middlewares.rend(); ++it) {
            auto& mw = *it;

            chain = [mw, next_link = std::move(chain)](
                        const types::request_t& req) -> shared::awaitable_t<types::response_t> {
...
TEST_F(middleware_tests, no_middleware) {
    run_awaitable([this]() -> boost::asio::awaitable<void> {
        ...
        auto chain = compose(
            middlewares, [this](const types::request_t& req) { return final_handler(req); });
        ...
        EXPECT_NE(hdrs.find("X-Body"), hdrs.end());
        EXPECT_EQ(hdrs.at("X-Body"), "final handler response");
    });
}
...
TEST_F(middleware_tests, single_middleware_modifies_response) {
    run_awaitable([this]() -> boost::asio::awaitable<void> {
        ...
        EXPECT_NE(hdrs.find("X-Header-A"), hdrs.end());
        EXPECT_EQ(hdrs.at("X-Header-A"), "Value-A");
    });
}
...
TEST_F(middleware_tests, multiple_middlewares_in_order) {
    run_awaitable([this]() -> boost::asio::awaitable<void> {
        ...
        EXPECT_NE(hdrs.find("X-Header-B"), hdrs.end());
        EXPECT_NE(hdrs.find("X-Header-C"), hdrs.end());

        EXPECT_EQ(hdrs.at("X-Header-B"), "Value-B");
        EXPECT_EQ(hdrs.at("X-Header-C"), "Value-C");
...
TEST_F(middleware_tests, middleware_short_circuits_chain) {
    run_awaitable([this]() -> boost::asio::awaitable<void> {
        ...
        EXPECT_NE(hdrs.find("X-Body"), hdrs.end());
        EXPECT_EQ(hdrs.at("X-Body"), "short-circuited");

        EXPECT_EQ(tracker_called_count, 0);

        EXPECT_NE(hdrs.find("X-Should-Be-Set"), hdrs.end());
        EXPECT_EQ(hdrs.at("X-Should-Be-Set"), "Value");
    });
}

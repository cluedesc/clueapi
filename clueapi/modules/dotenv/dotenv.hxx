...
namespace clueapi::modules::dotenv {
    ...
    class c_dotenv {
      public:
        CLUEAPI_INLINE c_dotenv() = default;

        CLUEAPI_INLINE ~c_dotenv() { destroy(); }
      ...
      public:
         ...
         * @param trim_values If `true`, whitespace will be trimmed from the start and end of values.
         */
      ...
      public:
         ...
         */
        CLUEAPI_INLINE std::size_t size() const { return m_container.size(); }
         ...
         */
        CLUEAPI_INLINE bool contains(const detail::hash_t& key) const { return m_container.contains(key); }
        ...
        CLUEAPI_INLINE _value_t at(const detail::hash_t& key, const _value_t& default_value) const {
            ...
            else if constexpr (std::is_integral_v<_value_t> || std::is_floating_point_v<_value_t>) {
                _value_t ret{};

                auto [ptr, ec] = std::from_chars(value_str.data(), value_str.data() + value_str.size(), ret);
    ...
    /**
     * @brief A dispatch helper to allow calling `get` on both pointers and references to `c_dotenv`.
     */
    template <typename _value_t, typename _env_t>
    CLUEAPI_INLINE _value_t
    get_dispatch(_env_t&& env, const detail::hash_t& key, const _value_t& default_value = _value_t{}) {

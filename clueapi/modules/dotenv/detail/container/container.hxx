...
namespace clueapi::modules::dotenv::detail {
    ...
    struct container_t {
        CLUEAPI_INLINE constexpr container_t() = default;

      public:
        /**
         * @brief Swaps the internal map with an empty one to release memory.
         */
        CLUEAPI_INLINE void destroy() { shared::unordered_map<_key_t, _value_t>().swap(m_container); }

      public:
        CLUEAPI_INLINE _value_t& operator[](const _key_t& key) noexcept { return m_container.at(key); }

        CLUEAPI_INLINE const _value_t& operator[](const _key_t& key) const noexcept { return m_container.at(key); }
      ...
      public:
         ...
         */
        CLUEAPI_INLINE _value_t& at(const _key_t& key) { return m_container.at(key); }
         ...
         * @throws std::out_of_range if the key does not exist.
         */
        CLUEAPI_INLINE const _value_t& at(const _key_t& key) const { return m_container.at(key); }
         ...
         */
        CLUEAPI_INLINE void set(const _key_t& key, const _value_t& value) { m_container.insert_or_assign(key, value); }
         ...
         */
        CLUEAPI_INLINE bool contains(const _key_t& key) const { return m_container.contains(key); }
         ...
         */
        CLUEAPI_INLINE void reserve(std::size_t size) { m_container.reserve(size); }
         ...
         */
        CLUEAPI_INLINE void erase(const _key_t& key) { m_container.erase(key); }

        /**
         * @brief Clears the container, removing all key-value pairs.
         */
        CLUEAPI_INLINE void clear() { m_container.clear(); }
         ...
         */
        CLUEAPI_INLINE const auto size() const { return m_container.size(); }

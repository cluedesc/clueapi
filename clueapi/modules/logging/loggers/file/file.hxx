...
namespace clueapi::modules::logging {
     ...
     */
    struct file_logger_t final : base_logger_t {
        /**
         * @brief Constructs a file logger with the given parameters.
         *
         * @param params The parameters for the logger.
         */
        CLUEAPI_INLINE file_logger_t(logger_params_t params) : base_logger_t{std::move(params)} {
            m_file_hash = detail::rt_hash(m_file_path);
        }

        CLUEAPI_INLINE ~file_logger_t() {
            m_buffer.clear();
      ...
      public:
         ...
         */
        CLUEAPI_INLINE std::string_view file_path() const { return m_file_path; }
         ...
         */
        CLUEAPI_INLINE void set_file_path(std::string path) {

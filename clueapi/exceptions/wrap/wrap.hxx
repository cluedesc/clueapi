 ...
 * @file wrap.hxx
 * 
 * @brief Provides functions to bridge exception-based code with `tl::expected`-based error handling.
 *
...
namespace clueapi::exceptions {
    ...
    CLUEAPI_INLINE tl::unexpected<_message_t> make_unexpected(_message_t msg = {}) {
     ...
     * that throws exceptions and converting the outcome into a modern, explicit error-handling model.
     */
    ...
    /**
     * @brief Executes an awaitable and safely wraps any thrown std::exception into an `expected_awaitable_t`.
     * 
     ...
     * @param ctx A context string to prepend to any resulting error message for diagnostics.
     * 
     * @return An awaitable that resolves to an `expected_t` containing the result or a formatted error.
     *
     ...
     * termination rather than being caught by a traditional try/catch block in the calling function.
     * This wrapper ensures all exceptions are caught and converted to an error value.
     */
    template <typename _ret_t = void, typename _awaitable_t>
    CLUEAPI_NOINLINE expected_awaitable_t<_ret_t> wrap_awaitable(_awaitable_t&& awaitable, std::string_view ctx) {
        try {
            if constexpr (std::is_void_v<_ret_t>) {
                (void)co_await std::forward<_awaitable_t>(awaitable);

                co_return expected_t<_ret_t>{};
            }
            else {
                if constexpr (std::is_invocable_v<_awaitable_t>) {
                    auto result = co_await std::forward<_awaitable_t>(awaitable)();

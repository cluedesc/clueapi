...
namespace clueapi::exceptions::detail {
    /**
     * @brief A customizable base exception class that automatically prepends a message with a compile-time prefix.
     *
     * @tparam _prefix A `prefix_holder_t` instance that provides the string prefix for the exception message.
     *
    ...
    struct base_exception_t : std::exception {
        ...
        CLUEAPI_INLINE base_exception_t(std::string_view msg, _args_t&&... args) {
            m_what = make(msg, std::forward<_args_t>(args)...);
        }

        CLUEAPI_INLINE virtual ~base_exception_t() = default;
      ...
      public:
         ...
         */
        CLUEAPI_INLINE const char* what() const noexcept override { return m_what.c_str(); }
         ...
         */
        template <typename... _args_t>
        CLUEAPI_INLINE static std::string make(std::string_view msg, _args_t&&... args) {
            ...
            if constexpr (sizeof...(_args_t) > 0) {
                ...
                if constexpr (prefix.empty()) {
                    fmt::format_to(std::back_inserter(buf), fmt::runtime(msg), std::forward<_args_t>(args)...);
                }
                else {

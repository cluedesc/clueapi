...
namespace clueapi::route::detail {
     ...
     */
    std::size_t longest_common_prefix(const std::string_view& a, const std::string_view& b) {
        std::size_t i{};
    ...
    std::optional<std::pair<_type_t, http::types::params_t>> radix_tree_t<_type_t>::find(
        ...
        while (true) {
            ...
            if (current_node->m_dynamic_child) {
                ...
                auto param_value = next_slash == std::string_view::npos ? path : path.substr(0, next_slash);
    ...
    template <typename _type_t>
    http::types::path_t radix_tree_t<_type_t>::norm_path(const http::types::path_t& path) {
        if (path.empty())
            return "/";
    ...
    template <typename _type_t>
    std::vector<std::string_view> radix_tree_t<_type_t>::split_path_segments(std::string_view path) {
    ...
    template <typename _type_t>
    std::string_view radix_tree_t<_type_t>::extract_param_name(std::string_view segment) {
    ...
    template <typename _type_t>
    bool radix_tree_t<_type_t>::is_dynamic_segment(std::string_view segment) {
        return segment.size() > 2 && segment.front() == '{' && segment.back() == '}';
    }

    template <typename _type_t>
    bool radix_tree_t<_type_t>::is_broken_segment(std::string_view segment) {

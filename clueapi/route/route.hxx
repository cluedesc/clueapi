...
namespace clueapi::route {
    ...
    struct base_route_t {
         ...
         */
        virtual bool is_awaitable() const = 0;
    };
    ...
    struct route_t : base_route_t {
        CLUEAPI_INLINE constexpr route_t() = default;
        ...
        CLUEAPI_INLINE route_t(
            const http::types::method_t::e_method& method, const http::types::path_t& path,

            _handler_t&& handler
        )
            : m_handler(std::move(handler)), m_path(path), m_method(method) {}

      public:
        CLUEAPI_INLINE route_t(const route_t&) = delete;

        CLUEAPI_INLINE route_t& operator=(const route_t&) = delete;
      ...
      public:
         ...
         * @return The response.
         */
        CLUEAPI_NOINLINE shared::awaitable_t<http::types::response_t> handle_awaitable(http::ctx_t ctx) override {
         ...
         * @return `true` if the route is awaitable, `false` otherwise.
         */
        CLUEAPI_INLINE bool is_awaitable() const override { return detail::handler_awaitable_c<_handler_t>; }

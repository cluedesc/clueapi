...
namespace clueapi::cfg {
     ...
     * 
     * @note These settings are only used if the logging module is enabled via `CLUEAPI_USE_LOGGING_MODULE`.
     */
    struct logging_cfg_t {
        /**
         * @brief Enables or disables asynchronous logging mode.
         *
         * @note When `true`, log messages are placed in a queue and processed by a dedicated background thread.
         * This improves application performance by offloading I/O operations.
         * When `false`, log messages are processed synchronously, which may block the calling thread.
         */
         ...
         *
         * @note In async mode, this is the duration the worker thread sleeps before checking for new messages.
         * Only applicable if `m_async_mode` is `true`.
         */
        std::chrono::milliseconds m_sleep{100};

        /**
         * @brief The default logging level. Messages with a severity lower than this will be ignored.
         */
         ...
         *
         * @warning If the queue is full, new log messages are typically dropped to prevent the application from
         * blocking. This is a critical parameter for balancing performance and log reliability under high load. Only
         * applicable if `m_async_mode` is `true`.
         */
         ...
         *
         * @note The worker thread will process up to this many messages from the queue in one go before sleeping.
         * Only applicable if `m_async_mode` is `true`.
         */

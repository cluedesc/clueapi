...
namespace clueapi::http::types {
     ...
     */
    struct request_t {
        CLUEAPI_INLINE constexpr request_t() = default;

        CLUEAPI_INLINE request_t(const request_t& other)
            ...
            if (other.m_cookies_parsed) {
                ...
                for (const auto& [key_view, value_view] : other.m_cookies) {
                    ...
                    std::string_view new_key_view(m_cookies_buffer.data() + key_offset, key_view.length());
                    std::string_view new_value_view(m_cookies_buffer.data() + value_offset, value_view.length());
        ...
        CLUEAPI_INLINE request_t& operator=(const request_t& other) {
            if (this == &other)
                return *this;
      ...
      public:
         ...
         */
        CLUEAPI_INLINE std::optional<std::string_view> cookie(const std::string_view& name) const {
            if (!m_cookies_parsed)
                parse_cookies();
         ...
         * otherwise`std::nullopt`.
         */
        CLUEAPI_INLINE std::optional<std::string_view> header(const std::string_view& name) const {
         ...
         */
        CLUEAPI_INLINE bool keep_alive() const {
        ...
        CLUEAPI_INLINE void reset() {
            m_method = method_t::e_method::unknown;
      ...
      public:
         ...
         */
        CLUEAPI_INLINE auto& method() { return m_method; }
         ...
         */
        CLUEAPI_INLINE const auto& method() const { return m_method; }
         ...
         */
        CLUEAPI_INLINE auto& uri() { return m_uri; }
         ...
         */
        CLUEAPI_INLINE const auto& uri() const { return m_uri; }
         ...
         */
        CLUEAPI_INLINE auto& parse_path() { return m_parse_path; }
         ...
         */
        CLUEAPI_INLINE const auto& parse_path() const { return m_parse_path; }
         ...
         */
        CLUEAPI_INLINE const auto& cookies() const {
            if (!m_cookies_parsed)
                parse_cookies();

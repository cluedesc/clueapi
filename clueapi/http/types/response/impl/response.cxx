...
namespace clueapi::http::types {
    ...
    file_response_t::file_response_t(boost::filesystem::path path, status_t::e_status status, headers_t headers) {
        try {
            if (!boost::filesystem::exists(path) || !boost::filesystem::is_regular_file(path)) {
                m_status = status_t::e_status::not_found;
...
#if BOOST_ASIO_HAS_IO_URING
            m_stream_fn = [file_size, path = std::move(path)](chunks::chunk_writer_t& writer
                          ) -> exceptions::expected_awaitable_t<> {
                auto executor = co_await boost::asio::this_coro::executor;

                boost::asio::stream_file file(executor, path.string(), boost::asio::stream_file::read_only);
                ...
                while (total_sent < file_size && !writer.writer_closed()) {
                    const auto to_read = std::min<std::size_t>(k_def_buffer_size, file_size - total_sent);

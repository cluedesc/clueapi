...
namespace clueapi::http::types {
    ...
    struct status_t {
         ...
         * @enum e_status
         *
         * @brief A type-safe enumeration of standard HTTP status codes.
         */
        enum struct e_status {
            continue_status     = 100,
         ...
         */
        CLUEAPI_INLINE static constexpr std::string_view to_str(e_status status) {
            switch (status) {
                // 1xx Informational
         ...
         */
        CLUEAPI_INLINE static constexpr std::string_view to_str(std::size_t status) {
            auto status_code = static_cast<e_status>(status);

            return to_str(status_code);
        }
    };
}

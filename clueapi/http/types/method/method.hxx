...
namespace clueapi::http::types {
     ...
     *
     * @details Provides a scoped enum `e_method` for type-safe representation of HTTP methods
     * and static constexpr functions for converting between the enum and its string representation.
     */
    struct method_t {
        /**
         * @enum e_method
         * @brief Scoped enumeration of standard HTTP methods.
         */

        enum struct e_method { unknown = 0, get, head, post, put, delete_, connect, options, trace, patch, count };
         ...
         */
        CLUEAPI_INLINE static constexpr std::string_view to_str(e_method method) {
            const auto idx = static_cast<std::size_t>(method);

            return idx < k_method_names.size() ? k_method_names[idx] : "UNKNOWN";
        }

        /**
         * @brief Converts a string representation of an HTTP method to its corresponding `e_method` enum value.
         *
         ...
         *
         * @details This function uses a switch on string length as a performance optimization for faster lookups.
         */
        CLUEAPI_INLINE static constexpr e_method from_str(std::string_view str) {
      ...
      private:
        /**
         * @brief A compile-time array holding the string names of HTTP methods, indexed by `e_method`.
         */
        static constexpr std::array<std::string_view, static_cast<size_t>(e_method::count)> k_method_names

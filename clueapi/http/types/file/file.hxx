...
namespace clueapi::http::types {
     ...
     */
    struct file_t {
        CLUEAPI_INLINE constexpr file_t() = default;
         ...
         */
        CLUEAPI_INLINE file_t(std::string name, std::string content_type, std::vector<char> data)
         ...
         */
        CLUEAPI_INLINE file_t(std::string name, std::string content_type, boost::filesystem::path temp_path)
      ...
      public:
         ...
         */
        CLUEAPI_INLINE std::size_t size() const {
            if (m_in_memory)
                return m_data.size();
      ...
      public:
         ...
         */
        CLUEAPI_INLINE const auto& content_type() const { return m_content_type; }

        /**
         * @brief Gets the path to the temporary file on disk.
         * @return A const reference to the `boost::filesystem::path`. Will be empty if the file is in memory.
         */
        CLUEAPI_INLINE const auto& temp_path() const { return m_temp_path; }

        /**
         * @brief Gets the file's content as a byte vector.
         * @return A const reference to the data vector. Will be empty if the file is stored on disk.
         */
        CLUEAPI_INLINE const auto& data() const { return m_data; }

        /**
         * @brief Checks if the file is stored in memory.
         * @return `true` if the file content is in memory, `false` if it is on disk.
         */
        CLUEAPI_INLINE const auto& in_memory() const { return m_in_memory; }
     ...
     */
    using files_t = shared::unordered_map<std::string, file_t, http::detail::sv_hash_t, http::detail::sv_eq_t>;
}

...
namespace clueapi::http::types {
    exceptions::expected_t<raw_cookie_t> cookie_t::serialize(const cookie_t& cookie) {
        ...
        if (cookie.m_name.starts_with("__Host-")) {
            if (!cookie.m_secure || !cookie.m_domain.empty() || cookie.m_path != "/") {
                return exceptions::make_unexpected(
                    exceptions::invalid_argument_t::make("__Host- cookies require secure, no domain, Path=/")
                );
            }
        }
        ...
        {
            const auto reserve = cookie.m_name.size() + cookie.m_value.size() + detail::k_cookie_buf_reserve;
    ...
    exceptions::expected_t<raw_cookie_t> cookie_t::serialize() { return serialize(*this); }
}
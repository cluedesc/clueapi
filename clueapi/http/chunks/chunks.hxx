...
namespace clueapi::http::chunks {
     ...
     *
     * @brief Manages the process of sending data in HTTP chunked transfer encoding format over a socket.
     *
    ...
    struct chunk_writer_t {
         ...
         */
        CLUEAPI_INLINE chunk_writer_t(boost::asio::ip::tcp::socket& socket)
            : m_final_chunk_written(false), m_socket(socket), m_buffer() {}

        CLUEAPI_INLINE ~chunk_writer_t() = default;
      ...
      public:
         ...
         *
         * @return An `exceptions::expected_awaitable_t<>` that resolves to an empty expected on success,
         * or an unexpected containing an error message on failure.
         *
         * @details The function formats the data by prepending its size in hexadecimal followed by CRLF,
         * and appending a final CRLF.
         */
        exceptions::expected_awaitable_t<> write_chunk(std::string_view data);

        /**
         * @brief Asynchronously writes the final zero-sized chunk to signify the end of the response body.
         *
         * @return An `exceptions::expected_awaitable_t<>` that resolves to an empty expected on success,
         * or an unexpected containing an error message on failure.
         *
         ...
         */
        exceptions::expected_awaitable_t<> write_final_chunk();
      ...
      public:
         ...
         */
        CLUEAPI_INLINE bool writer_closed() const { return !m_socket.is_open(); }
         ...
         */
        CLUEAPI_INLINE bool final_chunk_written() const { return m_final_chunk_written; }

      private:
        /**
         * @brief Flag indicating if the final chunk has been written.
         */
        bool m_final_chunk_written;
         ...
         *
         * @details This is a basic_memory_buffer with a default capacity of `k_def_buffer_size` bytes.
         */
      ...
      private:
         ...
         *
         * @details This is a static constant array of two characters, representing the CRLF sequence.
         */
